...
namespace sdf
{
  ...
  /// SDF of an Ellipsoid. Parameters to the constructor should contain the scalings along X, Y, Z and a global scale similar to a radius for the sphere.
  class Ellipsoid : public Primitive
  {
  public:
    Ellipsoid( operation_t op, Eigen::Affine3d &T, Eigen::Vector4d &params, bool inv=false)
    {
      invert = inv;
      operation = op;
      transformation = T;
    ...
    { 
      double value = 
        sqrt( (location(0)*location(0))/(parameters(0)*parameters(0)) + 
              (location(1)*location(1))/(parameters(1)*parameters(1)) +
              (location(2)*location(2))/(parameters(2)*parameters(2)) ) - (parameters(3));
  ...
  /// Ray-casts an image of size rows x cols of the SDF at a camera whose pose is given by the supplied transformation. Result is written to Normals
  void RenderNormal(
